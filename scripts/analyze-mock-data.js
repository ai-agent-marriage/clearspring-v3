...
function analyzeData() {
  ...
  Object.entries(userLevels).forEach(([level, count]) => {
    console.log(`    ${level}: ${count}人 (${(count/users.length*100).toFixed(1)}%)`);
  });
  ...
  console.log(`  总功德值：${totalMerit}`);
  console.log(`  人均订单：${(totalOrders/users.length).toFixed(1)}`);
  console.log(`  人均功德：${(totalMerit/users.length).toFixed(1)}\n`);
  
  // 执行者分析
  console.log('🎯 执行者数据分析:');
  const certifiedExecutors = executors.filter(e => e.qualificationStatus === '已认证').length;
  console.log(`  已认证：${certifiedExecutors}人 (${(certifiedExecutors/executors.length*100).toFixed(1)}%)`);
  console.log(`  审核中：${executors.length - certifiedExecutors}人`);
  ...
  console.log(`  平均完成订单：${avgCompleted.toFixed(1)}\n`);
  ...
  console.log(`  总志愿者数：${totalVolunteers}`);
  console.log(`  平均评分：${avgOrgRating.toFixed(2)}\n`);
  ...
  console.log(`  总服务时长：${totalServiceHours}小时`);
  console.log(`  总任务数：${totalTasks}`);
  console.log(`  平均评分：${avgVolRating.toFixed(2)}`);
  console.log(`  人均服务时长：${(totalServiceHours/volunteers.length).toFixed(1)}小时\n`);
  ...
  Object.entries(orderStatuses).forEach(([status, count]) => {
    console.log(`    ${status}: ${count}单 (${(count/orders.length*100).toFixed(1)}%)`);
  });
  ...
  Object.entries(serviceTypes).forEach(([type, count]) => {
    console.log(`    ${type}: ${count}单 (${(count/orders.length*100).toFixed(1)}%)`);
  });
  ...
  console.log('  物种分布 (前 10):');
  Object.entries(speciesCount)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .forEach(([species, count]) => {
      console.log(`    ${species}: ${count}单`);
    });
  ...
  console.log(`  平均金额：${avgAmount.toFixed(2)}元`);
  console.log(`  金额范围：${minAmount}-${maxAmount}元\n`);
  ...
  Object.entries(ratings).sort((a, b) => a[0] - b[0]).forEach(([rating, count]) => {
    ...
    console.log(`    ${stars} (${rating}星): ${count}条 (${(count/reviews.length*100).toFixed(1)}%)`);
  });
  
  const avgRating_review = reviews.reduce((sum, r) => sum + r.rating, 0) / reviews.length;
  const withReply = reviews.filter(r => r.reply).length;
  console.log(`  平均评分：${avgRating_review.toFixed(2)}`);
  console.log(`  有回复的评价：${withReply}条 (${(withReply/reviews.length*100).toFixed(1)}%)\n`);
  ...
  if (consistencyErrors === 0) {
    ...
    console.log(`  ❌ 发现 ${consistencyErrors} 个关联错误`);
  }
  ...
  if (timeErrors === 0) {
    ...
    console.log(`  ❌ 发现 ${timeErrors} 个时间逻辑错误`);
  }
