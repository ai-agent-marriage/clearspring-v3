...
async function testCreatePayIdempotency() {
  // [CLEANED] console.log('=== 测试用例 1：重复调用 createPay ===');
  ...
  try {
    // 第一次调用
    // [CLEANED] console.log('第一次调用 createPay...');
    ...
    // [CLEANED] console.log('第一次调用结果:', JSON.stringify(result1.result, null, 2));
    ...
    // [CLEANED] console.log('第二次调用结果:', JSON.stringify(result2.result, null, 2));
...
async function testPayCallbackIdempotency() {
  ...
  try {
    // 第一次回调
    // [CLEANED] console.log('第一次回调 payCallback...');
    ...
    // [CLEANED] console.log('第一次回调结果:', callback1.result);
    ...
    // [CLEANED] console.log('第一次回调后 pay_log 记录数:', logCount1.total);
    ...
    // [CLEANED] console.log('第二次回调结果:', callback2.result);
    ...
    // [CLEANED] console.log('第二次回调后 pay_log 记录数:', logCount2.total);
...
async function testSyncOrderStatus() {
  ...
  try {
    ...
    // [CLEANED] console.log('创建测试订单:', testOrderNo);
    
    // 调用同步函数
    // [CLEANED] console.log('调用 syncOrderStatus...');
    const syncResult = await cloud.callFunction({
      name: 'pay/syncOrderStatus'
    });
    
    // [CLEANED] console.log('同步结果:', JSON.stringify(syncResult.result, null, 2));
    ...
    // [CLEANED] console.log('订单状态:', orderQuery.data[0]?.status);
...
async function testTimeoutCancel() {
  ...
  try {
    ...
    // [CLEANED] console.log('创建过期测试订单:', testOrderNo);
    ...
    // [CLEANED] console.log('订单状态:', orderQuery.data[0]?.status, '(3=已取消)');
...
async function testRefundIdempotency() {
  ...
  try {
    ...
    // [CLEANED] console.log('创建已支付测试订单:', testOrderNo);
    
    // 第一次退款
    // [CLEANED] console.log('第一次调用 refund...');
    ...
    // [CLEANED] console.log('第一次退款结果:', JSON.stringify(refund1.result, null, 2));
    ...
    // [CLEANED] console.log('第二次退款结果:', JSON.stringify(refund2.result, null, 2));
...
exports.main = async (event, context) => {
  // [CLEANED] console.log('========================================');
  // [CLEANED] console.log('支付系统幂等性测试开始');
  // [CLEANED] console.log('========================================\n');
  ...
  // 汇总结果
  // [CLEANED] console.log('\n========================================');
  // [CLEANED] console.log('测试结果汇总');
  // [CLEANED] console.log('========================================');
  // [CLEANED] console.log('1. createPay 幂等性:', results.createPayIdempotency ? '✅ 通过' : '❌ 失败');
  // [CLEANED] console.log('2. payCallback 幂等性:', results.payCallbackIdempotency ? '✅ 通过' : '❌ 失败');
  // [CLEANED] console.log('3. 状态同步:', results.syncOrderStatus ? '✅ 通过' : '❌ 失败');
  // [CLEANED] console.log('4. 超时取消:', results.timeoutCancel ? '✅ 通过' : '❌ 失败');
  // [CLEANED] console.log('5. 退款幂等性:', results.refundIdempotency ? '✅ 通过' : '❌ 失败');
